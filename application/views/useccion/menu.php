<?php if (isset($menuTabla) && count($menuTabla)): ?>
    <ul class="nav nav-pills nav-stacked list-group sidebar-nav-v1" id="sidebar-nav">
        ...
        <?php for ($i = 0; $i < $numFilas; $i++): ?>
                    ...
                    <ul id="collapse-<?php echo ((isset($menuTabla[$i]['id'])) ? $menuTabla[$i]['id'] : "-"); ?>" class="collapse" aria-expanded="false" >
                        <?php while ($esSubmenu): ?>
                            <?php if ($menuTabla[$i]['tipo'] != 0): ?>
                                <li style="border:none;"><a href="/oferta-academica/<?php echo ((isset($idnom)) ? $idnom : "-"); ?>/<?php echo ((isset($menuTabla[$i]['href'])) ? $menuTabla[$i]['href'] : "-"); ?>" class="text-submenu" style="border:none; border-bottom: 1px solid #DDDEDF;"><i class="fa fa-angle-double-right" aria-hidden="true"></i> <?php echo ((isset($menuTabla[$i]['nombre'])) ? $menuTabla[$i]['nombre'] : "-"); ?></a></li>
                            <?php endif; ?>
                            <?php $i++; ?>
            ...
            <?php else: ?>
                <li class="list-group-item" style="border:none; border-bottom: 1px solid #DDDEDF;"><a href="/oferta-academica/<?php echo ((isset($idnom)) ? $idnom : "-"); ?>/<?php echo ((isset($menuTabla[$i]['href'])) ? $menuTabla[$i]['href'] : "-"); ?>" class="text-menu"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i> <?php echo ((isset($menuTabla[$i]['nombre'])) ? $menuTabla[$i]['nombre'] : "-"); ?></a></li>
                <?php endif; ?>
            <?php endfor; ?>
    </ul>
<?php endif; ?>