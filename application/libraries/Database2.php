        ...
        foreach ($info as $key => $value) {
            ...
            if (in_array($key, $fechas)) {
                if ($value == 'SYSDATE') {
                    ...
                    $values.= "TO_DATE('{$value}','DD/MM/YYYY HH:MI AM')" . (($key == $last_key) ? "" : ","); //yyyy/mm/dd hh:mi:ss AM
                }
            } else {
        ...
        foreach ($set as $key => $value) {
            if (in_array($key, $fechas)) {
                if ($value == 'SYSDATE') {
                    ...
                    $sql_set.= $key . "=TO_DATE('{$value}','DD/MM/YYYY HH:MI AM')" . (($key == $last_key_set) ? "" : ",");
                }
            } else {
        ...
        foreach ($where as $key => $value) {
            if (in_array($key, $fechas)) {
                if ($value == 'SYSDATE' || $value == 'TRUNC(SYSDATE)') {
                    ...
                    $sql_where.= $key . "=TO_DATE('{$value}','DD/MM/YYYY HH:MI AM')" . (($key == $last_key_where) ? "" : " AND ");
                }
            } else {
                $sql_where.= $key . "='" . $value . (($key == $last_key_where) ? "'" : "' AND ");
            }
